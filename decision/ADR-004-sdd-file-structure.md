# ADR-004: Canonical SDD File Structure — Org-Wide

**Status:** `draft`
**Date:** 2026-04-19
**Author:** Eric Caskey
**Depends on:**
- `ADR-001-spec-package-shapes.md` — integration vs. domain package shapes
- `ADR-002-two-domain-brand-split.md` — two-domain architecture
- `ADR-003-spec-driven-development.md` — SDD as the default process

---

## Context

The specs repo has grown organically. The two-domain split (ADR-002) was established in
April 2026 and created `ericcaskey-com/` and `caskeycoding-com/` workspaces — but the
existing flat directories (`frontend/`, `backend/`, `infra/`, `architecture/`,
`case-studies/`, `content-strategy/`) predate that split and have not been moved.
...
`marathon-coach` is a domain package (also deliberate per ADR-001), and both have proper
...
│   └── ADR-004-sdd-file-structure.md      ← THIS FILE
│
...
| `projects/marathon-coach/` | Deliberate domain package per ADR-001 — do not touch structure |
...
The two-domain split (ADR-002) established that cross-cutting concerns live in `_shared/`
and org-wide decisions live in `decision/`. This ADR preserves those rules.
...
|-------------|------|------|
| Keep flat structure forever | Zero migration cost | Routing table is wrong; violates domain ownership from ADR-002 |
...
- [ ] `decision/ADR-004-sdd-file-structure.md` merged to master
