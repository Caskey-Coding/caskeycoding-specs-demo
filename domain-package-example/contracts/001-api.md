...
> Two surfaces: the CLI (humans at terminals) and the local HTTP API
> (the bundled web UI). Both target the same domain operations
> defined in [`../domain/001-model.md`](../domain/001-model.md).
...
(invariant from [`../domain/001-model.md §Invariants`](../domain/001-model.md)).
...
`--reason` is required when `--to abandoned` (per the model invariant).
...
All JSON. Timestamps RFC 3339 UTC. Versioning: URL prefix is `/api`
without a version — single consumer, breaking changes are fine
before 1.0. Post-1.0, move to `/api/v1`.
...
- [Domain model](../domain/001-model.md) — every CLI verb maps to
  one or more events listed here
- [Data contracts](../domain/002-data-contracts.md) — on-disk
  encoding behind these APIs
- [Use cases](../product/002-use-cases.md) — narrative flows that
  exercise these endpoints
